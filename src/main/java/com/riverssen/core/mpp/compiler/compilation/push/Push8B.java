...
public class Push8B implements CompilationStep
{
    @Override
    public void execute(Token token, AbstractSyntaxTree.CompileType type, AbstractSyntaxTree tree, Object... objects)
    {
        ...
        {
            case "0":
                tree.getOpcode().add(new Opcode(Ops.lconst_0));
                break;
            case "1":
                tree.getOpcode().add(new Opcode(Ops.lconst_1));
                break;
            case "2":
                tree.getOpcode().add(new Opcode(Ops.lconst_2));
                break;
            case "3":
                tree.getOpcode().add(new Opcode(Ops.lconst_3));
                break;
                default:
                    tree.getOpcode().add(new Opcode(Ops.lconst).add(Opcode.convertLong(Long.parseLong(token.toString()))));
                    break;
        }
    }
}