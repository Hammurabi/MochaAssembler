...
public class Push4Bf implements CompilationStep
{
    @Override
    public void execute(Token token, AbstractSyntaxTree.CompileType type, AbstractSyntaxTree tree, Object... objects)
    {
        ...
        {
            case "0":
                tree.getOpcode().add(new Opcode(Ops.fconst_0));
                break;
            case "1":
                tree.getOpcode().add(new Opcode(Ops.fconst_1));
                break;
            case "2":
                tree.getOpcode().add(new Opcode(Ops.fconst_2));
                break;
            case "3":
                tree.getOpcode().add(new Opcode(Ops.fconst_3));
                break;
                default:
                    tree.getOpcode().add(new Opcode(Ops.fconst).add(Opcode.convertFloat(Double.parseDouble(token.toString()))));
                    break;
        }
    }
}